    ...
    <nav class="navbar fixed-top navbar-light bg-white topnav px-2 py-0" style="height: 108px;">
        <div class="col-3" style="height:70px">
            <div class="row mx-0">
                <div class="col-3 p-0">
                    @php
                    $logo = DB::table('ministry_logo')->where('id',auth()->guard('inventory')->user()->ministry_logo_id)->take(1)->value('logo')
                    @endphp
                ...
                <div class="col-9 pl-2 p-0 d-flex align-items-center">
                    <span>
                        <b class="text-nowrap"
                            style="color: #108f68;font-size: 18px">{{str_replace('MOH - ','',DB::table('hss_master')->where('hss_master_no', auth()->guard('inventory')->user()->hss_master_no)->take(1)->value('name1'))}}</b>
        ...
        <div class="col-6 text-center px-0">
            <h4 class="text-danger ff_mon">
                ...
                <b class="text_shadow">{{auth()->guard('inventory')->user()->name}}</b>
            </h4>
        </div>
                ...
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item my-1 ">
                        ...
                        <a class="nav-link" href="{{ route('inventory.display.order') }}">
                            <i class="fas fa-tachometer-alt fs_18"></i>&ensp; Display Documents</a>
                    </li>
                </ul>
            </div>
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent" style="height: 57px">
                        ...
                        <ul class="navbar-nav ml-auto">
                            ...
                            <li class="nav-item px-2 py-1">
                                <a href="{{ route('inventory.logout') }}" id="logout_a"><span
                                        class="badge badge-dark p-2" title="logout"><i
                                            class="fas fa-power-off"></i></span>
                                </a>
                                <form id="logout-form" action="{{ route('inventory.logout') }}" method="POST"
